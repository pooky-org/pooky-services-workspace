...
@QueryHandler(GetRssProductQuery)
export class GetRssProductHandler implements IQueryHandler<GetRssProductQuery> {
	constructor(
		private readonly rssParserService: TldrRssParserService,
		private readonly redisService: RedisService,
	) {}

	async execute(query: GetRssProductQuery): Promise<IParsedRss[]> {
		const intervalTime =
			Number(process.env.RSS_FETCH_INTERVAL_IN_MINUTES) || 60;
		const redisKey = `rss:product:${query.path || "product"}`;
