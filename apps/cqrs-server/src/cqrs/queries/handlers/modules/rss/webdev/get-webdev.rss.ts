...
@QueryHandler(GetRssWebdevQuery)
export class GetRssWebdevHandler implements IQueryHandler<GetRssWebdevQuery> {
	constructor(
		private readonly rssParserService: TldrRssParserService,
		private readonly redisService: RedisService,
	) {}

	async execute(query: GetRssWebdevQuery): Promise<IParsedRss[]> {
		const intervalTime =
			Number(process.env.RSS_FETCH_INTERVAL_IN_MINUTES) || 60;
		const redisKey = `rss:webdev:${query.path || "webdev"}`;
